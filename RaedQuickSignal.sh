#!/bin/sh
##setup command=wget http://tunisia-dreambox.info/TSplugins/RaedQuickSignal/installer.sh -O - | /bin/sh

version=13.5
description=What is NEW:\n[Try to fix Crash from E2 source of VIX and OBH]\nما هو الجديد:\n[OBH و VIX محاولات إصلاح أخطاء ناتجة من سورس الانجيما في صور]
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   echo ""
   echo ""
   wget "http://tunisia-dreambox.info/TSplugins/RaedQuickSignal/RaedQuickSignal-"$version".tar.gz"
   wget "http://tunisia-dreambox.info/TSplugins/RaedQuickSignal/RaedQuickServName2-dreamos.tar.gz"
